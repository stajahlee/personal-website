...
const AnswerCard: FC<{ answer: string; onClick: () => void }> = ({ answer, onClick }) => {
  return (
...
export default function ChristmasQuestionPage() {
  ...
  if (isNaN(questionNumber) || questionNumber < 1 || questionNumber > questionsWithAnswers.length) {
    notFound();
  }
  ...
  const handleAnswerClick = (selectedAnswer: string) => {
    ...
    if (questionNumber < questionsWithAnswers.length) {
      router.push(`/christmas/${questionNumber + 1}?score=${newScore}&startTime=${startTime}`);
    } else {
      ...
      <div className="container flex flex-col items-center justify-center min-h-screen p-[24px] mx-auto gap-12">
        ...
        <div className="grid w-full grid-cols-1 gap-8 mx-auto mt-32 sm:mt-0 sm:grid-cols-3 lg:gap-16">
          {currentQuestion.options.map((option, index) => (
            <AnswerCard key={`${index}_${option}`} answer={option} onClick={() => handleAnswerClick(option)} />
          ))}
        </div>
      </div>
    </div>
